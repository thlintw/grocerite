...
{#if $dialog.showDialog}
    ...
    <div transition:scaleFade
        ...
        <div class="pointer-events-auto z-[11002] flex flex-col min-w-[25rem]
            ">
            ...
            <div class="bg-orange-50 rounded-2xl flex flex-col gap-3 px-3 py-3 pt-4
                ...
                <span class="text-center w-full mt-4">
                    { $_($dialog.message)}
                </span>
                
                <div class="flex gap-3 justify-center mt-3 w-8/12 mb-1">
                    {#if $dialog.onCancel}
                        <button type="button" class="border-orange-500 border-2 text-orange-500 rounded-llg py-0.5 basis-1/2"
                            on:click={$dialog.onCancel}>
                            { $_($dialog.cancelText) }
                        </button>
                    {/if}
                    {#if $dialog.onConfirm}
                        <button type="button" class="bg-orange-500 text-white rounded-llg py-1 basis-1/2"
